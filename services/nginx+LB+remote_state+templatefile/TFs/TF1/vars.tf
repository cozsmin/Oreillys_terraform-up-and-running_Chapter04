...
#!/bin/bash
printf "\n\n`date` :\n\n" >> /tmp/user_data
/usr/bin/sudo  /usr/bin/bash -c "/usr/bin/apt update && /usr/bin/sudo /usr/bin/apt install -y nginx dmidecode" >> /tmp/user_data 2>&1
/usr/bin/sudo  /usr/bin/bash -c "/usr/bin/printf \"`hostname` : `date` \n\n`dmidecode -t1`\n\n\" > /var/www/html/index.html " >> /tmp/user_data 2>&1
/usr/bin/sudo  /usr/bin/bash -c "/usr/bin/systemctl enable --now nginx" &> /tmp/user_data >> /tmp/user_data 2>&1
EOF
}
